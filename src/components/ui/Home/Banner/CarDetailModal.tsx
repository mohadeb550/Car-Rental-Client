...
export default function CarDetailModal ({ setIsOpen,  carId}) {
  ...
  if(dataLoading){
    return <div className="w-full h-full absolute top-0 left-0 right-0 bottom-0 bg-white/80 rounded-md flex justify-center items-center"> 
    <ClipLoader
       color='#000002'
  ...
  return (
    ...
    <div className="p-6 pt-0 ">
      <h5
        className="mb-4 text-2xl font-semibold leading-tight text-neutral-600 dark:text-neutral-50 text-center">
    {car?.name}
      </h5>

       <h5
      className="mb-4 text-[20px] flex items-center justify-center gap-2 font-semibold leading-tight text-gray-500 italic dark:text-neutral-50 text-center "> COST : 
   <span className="text-xl text-amber-500 font-racing not-italic">{car.pricePerHour}$ Per Hour</span> 
    </h5>

    <div className="flex gap-3 items-center justify-center">
      <span className='text-zinc-600 inter-regular text-lg'>Location : {car.location}</span>
    </div>
    
    <div className="flex gap-3 items-center justify-center">
      <span className='text-zinc-600 inter-regular text-lg'> Status :  {car.status}</span>
    </div>
