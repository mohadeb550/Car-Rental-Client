...
import { FaIdBadge, FaIdCardAlt, FaMailBulk } from "react-icons/fa";
import { FaIdCard, FaIdeal } from "react-icons/fa6";
...
const UserOverview = () => {
    ...
    return (
        ...
        <section className="flex justify-end items-center gap-1 font-play">
          ...
          {/* custom modal  */}
           ...
           <form className="w-[400px] md:w-[500px] p-7 bg-[#212433] rounded-lg" onSubmit={handleSubmit}>
    ...
    <div className="mb-3">
        <label className="text-lg text-gray-400"> Change Photo </label>
          <input name="image" defaultValue={user?.image}
        className="relative bg-gray-800 m-0 block mt-1 w-full min-w-0 flex-auto rounded border border-solid border-neutral-700 bg-clip-padding px-3 py-[0.32rem] text-base font-normal text-gray-300 transition duration-300 ease-in-out file:-mx-3 file:-my-[0.32rem] file:overflow-hidden file:rounded-none file:border-0 file:border-solid file:border-inherit file:bg-neutral-100 file:px-3 file:py-[0.32rem] file:text-neutral-700 file:transition file:duration-150 file:ease-in-out file:[border-inline-end-width:1px] file:[margin-inline-end:0.75rem] hover:file:bg-neutral-200 focus:border-primary  focus:shadow-te-primary focus:outline-none dark:border-neutral-600 dark:text-neutral-200 dark:file:bg-neutral-700 dark:file:text-neutral-100 dark:focus:border-primary"
        type="text"
        id="formFileMultiple"
         />
        </div>
    ...
    <button onClick={() => setOpen(!open)} className="text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm mt-2 md:ml-3  px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"> Close </button>
        ...
        <div className="p-8 bg-[#171A21] rounded-lg shadow mt-24 flex flex-col lg:flex-row">
          ...
          <div className="mt-28  border-b pb-12">
            ...
            <p className="mt-4 text-zinc-400 whitespace-nowrap flex items-center gap-2"> <FaIdCard className="text-xl"/> User Id :{user?._id}</p>
            <p className="mt-3 text-zinc-400 flex items-center gap-2"><TfiEmail className="text-xl"/> Email - {user?.email}</p>
            <p className="mt-3 text-zinc-400 capitalize flex items-center gap-2"><TbBadgesFilled className="text-xl"/> Role - {user?.role}</p>
          </div>
